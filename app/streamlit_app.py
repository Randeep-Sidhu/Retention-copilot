...
from src.value import policy_comparison, profit_curve  # noqa: E402
...
def trace_detail(e: dict) -> tuple[str, str]:
    n = e["node"]
    if n == "load_customer":
        return (e.get("error") or f"{e.get('contract')}, {e.get('internet_service')}, tenure {e.get('tenure_months')} months"), ""
    if n == "gate":
    ...
    if n == "finalize":
        return f"status: {e['status']}", "good" if e["status"] in ("queued", "no_action") else "bad" if e["status"] in ("escalated", "error", "failed_schema") else ""
    return "", ""


def render_run(run: dict, prof: dict | None, corpus: dict) -> None:
    prop = run.get("proposal")
    left, mid, right = st.columns([1.05, 1.55, 1.25], gap="medium")
    with left:
        if prof:
            ui.card("Customer", [("Contract", prof["contract"]), ("Internet", prof["internet_service"]),
                                 ("Tech support", prof["tech_support"]), ("Tenure", f"{prof['tenure_months']} months"),
                                 ("Monthly charges", f"${float(prof['monthly_charges']):.2f}"),
                                 ("Marketing consent", "yes" if int(prof["marketing_opt_in"]) else "no"),
                                 ("Contacts, last 90 days", str(prof["contacts_last_90d"]))])
            ...
            st.markdown(f'<div class="rc-card"><h4>Main factors behind the score</h4>{chips or "none notable"}</div>',
                        unsafe_allow_html=True)
    with mid:
        st.markdown(ui.status_pill(run["status"]) + ui.pill(f"{run.get('llm_calls', 0)} model call(s)", "blue") +
                    ui.pill(f"{run.get('attempts', 0)} revision(s)", "blue"), unsafe_allow_html=True)
        if prop:
        ...
        if first:
            st.markdown("**What the verifier caught in the first draft**")
        ...
        if run.get("violations"):
            st.markdown("**Still failing after the revisions (the reviewer is warned)**")
...
def page_overview():
    ...
    ui.hero("Retention Copilot",
            "A calibrated churn model decides who is worth contacting. A LangGraph agent drafts the offer from the company's own "
            "policy documents, a rule-based verifier checks every draft, and a person approves the message before anything is sent.",
            ("Python", "scikit-learn", "LangGraph", "RAG", "IBM Granite 4", "SQLite", "Streamlit"))
    c = st.columns(4)
    if m:
        ev = next((p for p in m["policies"] if p["policy"].startswith("Expected")), None)
        ui.kpi(c[0], "Churn model, ROC-AUC", f"{m['logreg']['roc_auc']:.3f}", f"held-out test set, n = {m['n_test']:,}")
    ...
    ui.kpi(c[2], "Policy sections", str(len(data.corpus())), "one spec generates the documents and the verifier")
    finals = [(model, rep["configs"]["rag+verify"]) for model, rep in reports.items() if "rag+verify" in rep["configs"]]
    if finals:
        rates = [cfg["final_ok"]["rate"] for _, cfg in finals]
        lo, hi = min(rates), max(rates)
        value = f"{100 * hi:.0f}%" if hi - lo < 1e-9 else f"{100 * lo:.0f}-{100 * hi:.0f}%"
        ui.kpi(c[3], "Drafts passing every rule", value,
               f"retrieval + verifier, {len(finals)} model(s), n = {finals[0][1]['final_ok']['n']}")
    else:
        ui.kpi(c[3], "Drafts passing every rule", "n/a", "run python -m src.evaluate")
    st.write("")
    ui.heading("How a customer moves through the system")
    ...
    for i, (col, (t, d)) in enumerate(zip(cols, steps), 1):
        ui.step(col, i, t, d)
    st.write("")
    ui.note("Read it in this order: Churn model and business case, then Agent replay, then Evaluation. "
    ...
    with st.expander("What this is and what it is not"):
        st.markdown(
            "- The data is IBM's public Telco churn sample. Two fields (marketing consent and recent contacts) are simulated so the compliance rules have something to test.\n"
            "- The company policy is fictional. \"Compliant\" here means the draft passes my rules, not that it satisfies any real regulator.\n"
            "- There is no offer/no-offer experiment in the data, so the business-value figures rest on assumptions. The what-if page shows how much they move.\n"
            "- The evaluation is small (tens of customers per configuration) and the models were run at temperature 0 on a free GPU.")


def page_model():
    m = data.metrics(str(data.path("reports")))
    ui.heading("Churn model and business case", "Who is at risk, who is worth contacting, and how sensitive that answer is to my assumptions.")
    if not m:
    ...
    with tab1:
        ...
        ui.note(f"Paired bootstrap, gradient boosting minus logistic: ROC-AUC {d['mean']:+.4f} (95% CI {d['lo']:+.4f} to {d['hi']:+.4f}). "
                "The interval includes zero, so I shipped the model I can explain per customer.")
        a, b = st.columns(2)
        fig = data.path("reports") / "figures"
        if (fig / "calibration.png").exists():
            a.image(str(fig / "calibration.png"), caption="Calibration: predicted vs observed churn on held-out customers")
        if (fig / "drivers.png").exists():
            b.image(str(fig / "drivers.png"), caption="What drives the predicted risk")
        try:
            t = data.tenure_table(str(data.path("db")))
            chart = alt.Chart(t).mark_bar(color=BLUE).encode(
                x=alt.X("band:N", sort=list(t["band"]), title="Tenure (months)"),
                y=alt.Y("churn_rate:Q", title="Churn rate", axis=alt.Axis(format="%")),
                tooltip=["band", "customers", alt.Tooltip("churn_rate:Q", format=".1%")]).properties(height=240)
            st.markdown("**Churn falls steeply and non-linearly with tenure, which is why the model uses tenure bands**")
            st.altair_chart(chart, use_container_width=True)
        except Exception:
            pass
    with tab2:
        pol = pd.DataFrame(m["policies"]).rename(columns={"policy": "Policy", "contacted_pct": "Contacted %", "churners_reached_pct": "Churners reached %",
                                                         "precision_pct": "Precision %", "net_value_per_1000": "Net value per 1,000 ($)"})
        st.dataframe(pol.round(1), hide_index=True, use_container_width=True)
        ui.note("Contacting a customer only when expected value is positive beats both contacting everyone and a fixed top-20% list. "
                "That ranking depends on the economic assumptions below.")
        fig = data.path("reports") / "figures"
        a, b = st.columns(2)
        if (fig / "profit_curve.png").exists():
            a.image(str(fig / "profit_curve.png"))
        if (fig / "sensitivity.png").exists():
            b.image(str(fig / "sensitivity.png"))
        econ = m["economics"]
    ...
    with tab3:
        ...
        st.markdown("Move the sliders and the contact policy is re-evaluated on the held-out customers. "
        ...
        line = alt.Chart(curve).mark_line(color=BLUE, strokeWidth=2.5).encode(
        ...
        dot = alt.Chart(pts).mark_point(color="#da1e28", size=140, filled=True).encode(x="pct_contacted:Q", y="net_value_per_1000:Q")
        zero = alt.Chart(pd.DataFrame({"y": [0]})).mark_rule(color=GREY, strokeDash=[4, 4]).encode(y="y:Q")
        st.altair_chart((line + dot + zero).properties(height=340), use_container_width=True)
...
def page_agent():
    ...
    if not sets:
        ...
        order = {"escalated": 0, "queued": 1, "queued_unverified": 2, "no_action": 3}
        pick.sort(key=lambda r: (order.get(r["status"], 9), r["customer_id"]))
        labels = {r["customer_id"]: f"{r['customer_id']}   [{r['status']}, {r['attempts']} revision(s)]" for r in pick}
        with tab1:
        ...
        with tab2:
            st.markdown(f"Customer **{cid}**, model `{model}`: the same customer under every configuration that was run.")
            rows = []
    ...
    with st.expander("Load sample drafts from the evaluation runs", expanded=not store.queue(None)):
        ...
        if st.button("Load into the queue", disabled=not sets):
            ...
            for r in data.runs(rs["path"], model):
                ...
                if added >= n:
                    break
            st.success(f"Added {added} draft(s) from {model}.")
            st.rerun()
        if not sets:
        ...
        for item in pending:
            ...
            with st.container(border=True):
                ...
                with c:
                    ...
                    ok, no = st.columns(2)
                    if ok.button("Approve", key=f"ok_{item['queue_id']}", type="primary", use_container_width=True):
                        store.decide(item["queue_id"], "approved", note)
                        st.rerun()
                    if no.button("Reject", key=f"no_{item['queue_id']}", use_container_width=True):
...
def grouped_bars(rep: dict):
    rows = []
    for cfg in CONFIG_ORDER:
        c = rep["configs"].get(cfg)
        if not c:
            continue
        for stage, key in (("first draft", "first_ok"), ("final", "final_ok")):
            rows.append({"config": cfg, "stage": stage, "rate": c[key]["rate"], "lo": c[key]["ci"][0], "hi": c[key]["ci"][1]})
    df = pd.DataFrame(rows)
    order = [c for c in CONFIG_ORDER if c in df["config"].unique()]
    base = alt.Chart(df).encode(x=alt.X("config:N", sort=order, title=None), xOffset="stage:N")
    bars = base.mark_bar().encode(y=alt.Y("rate:Q", title="Drafts passing every rule", axis=alt.Axis(format="%"), scale=alt.Scale(domain=[0, 1.05])),
                                  color=alt.Color("stage:N", scale=alt.Scale(domain=["first draft", "final"], range=["#a6c8ff", BLUE]), title=None),
                                  tooltip=["config", "stage", alt.Tooltip("rate:Q", format=".0%")])
    bars_err = base.mark_errorbar(color="#161616").encode(y="lo:Q", y2="hi:Q")
    return (bars + bars_err).properties(height=320)
...
def page_eval():
    ...
    ui.heading("Evaluation", "The same customers under different policy sources (none, full text, retrieved), with and without the verifier loop.")
    if not reports:
    ...
    ui.note("Compliant means no rule violation from the deterministic verifier. A final draft that went through the verifier loop is compliant "
            "almost by construction, so first-draft compliance shows the effect of the policy text and the model. "
            "'Offer matches playbook' asks a different question: did it choose the offer the segment playbook prescribes?")
    rows = []
    for model, m in reports.items():
        for cfg in CONFIG_ORDER:
            c = m["configs"].get(cfg)
            if c:
                rows.append({"Model": model, "Configuration": cfg, "First draft compliant": rate_text(c["first_ok"]),
                             "Final compliant": rate_text(c["final_ok"]), "Offer matches playbook": rate_text(c["matches_preferred"]),
                             "Prompt tokens": round(c["mean_prompt_tokens"]), "Model calls": round(c["mean_llm_calls"], 2),
                             "Model time / run (s)": round(c["mean_llm_latency_s"], 1)})
    st.dataframe(pd.DataFrame(rows), hide_index=True, use_container_width=True)
    tabs = st.tabs(list(reports))
    for tab, (model, m) in zip(tabs, reports.items()):
        with tab:
            st.caption(f"{kind} evaluation, {m['n']} customers per configuration, same customers for every configuration.")
            st.altair_chart(grouped_bars(m), use_container_width=True)
            ...
            st.markdown("**Paired comparisons on the same customers**")
            con["Exact McNemar p"] = con["Exact McNemar p"].map(lambda v: "< 0.0001" if v < 0.0001 else f"{v:.4f}")
            st.dataframe(con, hide_index=True, use_container_width=True)
            rules = pd.DataFrame(m["rule_rates"]).fillna(0.0)
            if not rules.empty:
                st.markdown("**Which rules the first drafts break** (share of customers)")
                st.dataframe(rules.style.format("{:.0%}").background_gradient(cmap="Blues", vmin=0, vmax=1), use_container_width=True)
...
def page_policy():
    ...
    with t2:
        ...
        if q.strip():
            hits = data.retriever("tfidf").search(q, k=k)
...
with st.sidebar:
    st.markdown("## Retention Copilot")
    st.caption("Churn to reviewed message, with the workings shown.")
